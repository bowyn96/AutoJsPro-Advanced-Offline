...
exports.playMusic = exports.scanFile = exports.MediaPlayer = void 0;
const path_1 = __importDefault(require("path"));
const lang_1 = require("./lang");
const rhino_1 = require("./rhino");
const android = rhino_1.Packages.android;
const connection = new android.media.MediaScannerConnection($autojs.androidContext, new android.media.MediaScannerConnection.MediaScannerConnectionClient($autojs.java.wrap({
    onMediaScannerConnected: function () {
    },
    onScanCompleted: function () {
    }
})));
connection.connect();
/**
...
class MediaPlayer {
    constructor() {
        this.prepareDeferred = Array();
        this.seekDeferred = Array();
        this.exitListener = () => {
            this.player.release();
        };
        this.player = new android.media.MediaPlayer();
        this.player.setOnPreparedListener(new android.media.MediaPlayer.OnPreparedListener($autojs.java.wrap({
            onPrepared: () => {
                this.onPrepared();
            }
        })));
        this.player.setOnSeekCompleteListener(new android.media.MediaPlayer.OnSeekCompleteListener($autojs.java.wrap({
            onSeekComplete: () => {
                this.onSeeked();
            }
        })));
        this.player.setOnCompletionListener(new android.media.MediaPlayer.OnCompletionListener($autojs.java.wrap({
            onCompletion: () => {
                var _a;
                (_a = this.completionDeferred) === null || _a === void 0 ? void 0 : _a.resolve();
            }
        })));
     ...
     */
    play(uri, volume, looping) {
        return __awaiter(this, void 0, void 0, function* () {
            this.reset();
            this.setDataSource(uri);
            this.setVolume(volume !== null && volume !== void 0 ? volume : 1.0);
            this.setLooping(looping !== null && looping !== void 0 ? looping : false);
            yield this.prepare();
            this.start();
        });
    }
    ...
    setVolume(leftVolume, rightVolume) {
        this.player.setVolume(leftVolume, rightVolume !== null && rightVolume !== void 0 ? rightVolume : leftVolume);
    }
    /**
     * @中文
     *
    ...
    prepare() {
        ...
        this.prepareDeferred.push(deferred);
    ...
    seekTo(msec) {
        const deferred = new lang_1.Deferred();
        this.seekDeferred.push(deferred);
        this.player.seekTo(msec, android.media.MediaPlayer.SEEK_CLOSEST);
        return deferred.promise();
    }
    /**
     * @中文
    ...
    start() {
        this.player.start();
        this.completionDeferred = new lang_1.Deferred();
    }
    /**
     * @中文
     *
    ...
    setScreenOnWhilePlaying(keep) {
        this.player.setScreenOnWhilePlaying(keep);
    }
    /**
     * @中文
     *
    ...
    awaitForCompletion() {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            yield ((_a = this.completionDeferred) === null || _a === void 0 ? void 0 : _a.promise());
        });
    }
    onPrepared() {
        this.resolveAll(this.prepareDeferred, undefined);
    }
    onSeeked() {
        this.resolveAll(this.seekDeferred, undefined);
    }
    resolveAll(deferredArray, value) {
        deferredArray.forEach(deferred => {
            deferred.resolve(value);
        });
        deferredArray.splice(0, deferredArray.length);
    }
}
...
function scanFile(file) {
    ...
    const mimeType = android.webkit.MimeTypeMap.getSingleton().getMimeTypeFromExtension(extension);
    if (!mimeType) {
        connection.scanFile(file, mimeType);
    }
    else {
        connection.scanFile(file, "*/*");
    }
}
 ...
 * ```
 */
function playMusic(uri, volume, looping) {
    return __awaiter(this, void 0, void 0, function* () {
        const player = new MediaPlayer();
        yield player.play(uri, volume, looping);
        return player;
    });
}
